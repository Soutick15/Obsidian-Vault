## 1. Executive Summary
NyAi is a sophisticated, AI-powered Compliance and Master Data Management (MDM) platform. The system is designed to provide centralized compliance tracking, intelligent risk assessment, and secure distribution of legal/compliance obligations to multiple tenants (cloud-hosted or on-premise). 
...
Across all microservices, NyAi standardizes on a highly modern, cloud-native stack:
...
The system is divided into two primary domains: **Compliance (Tenant-facing)** and **MDM (Master Data Management)**.
...
*Note: While many enterprise architectures rely heavily on Kafka/RabbitMQ or FeignClients for inter-service communication, NyAi specifically designed `mdm-core` to avoid these for data distribution to maximize performance.*
...
Across the NyAi platform, the codebase implements a mix of classic Gang of Four (GoF) design patterns and modern Microservice patterns to maintain clean, scalable, and resilient code.
...
- **Shared Database / Data Pump Pattern**: Used between `mdm-core` and `mdm-subscriptions`, and `compliance-auth` and `compliance-core`. While sharing databases is usually an anti-pattern, NyAi uses it to avoid network latency and HTTP timeouts when querying and aggregating millions of compliance records.
